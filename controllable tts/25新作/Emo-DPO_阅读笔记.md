...
|  +-------------------------------------------+   |
|  |  Input:  Spk + Happy + Text               |   |
|  |           ↓                               |   |
|  |  Speech Tokenizer → Happy speech tokens   |   |
|  |           ↓                               |   |
|  |  Emotion-aware LLM-TTS → Happy predictions|   |
|  |           ↓                               |   |
...
|  (b) Emo-DPO Training                            |
|  +-------------------------------------------+   |
|  |  Positive:  Happy input → Happy predictions  |
|  |  Negative:  Neutral input → Neutral pred.    |
|  |           ↓                               |   |
|  |  Emotion-aware LLM-TTS (shared π_θ)       |   |
|  |           ↓                               |   |
|  |  L_DPO + L_KL + L_SFT                     |   |
|  +-------------------------------------------+   |
+--------------------------------------------------+
...
|  |  Input:  Spk + Happy + Text               |   |
|  |           ↓                               |   |
|  |  Emotion-aware LLM-TTS → Happy speech tokens|
|  |           ↓                               |   |
|  |  Flow-matching Vocoder → Happy speech     |   |
|  +-------------------------------------------+   |
+--------------------------------------------------+
```
...
| 红线项 | 检查结果 | 说明 |
|--------|---------|------|
| **术语检验** | ✅ 通过 | "DPO"、"instruction tuning"、"KL divergence"、"Jensen-Shannon divergence" 等术语与论文原文一致。"Emotion-aware LLM-TTS" 为论文中使用的概念。 |
| **逻辑完整** | ⚠️ 存疑 | 论文假设"DPO 能捕捉细微情感差异"，但未直接证明模型学到的是"细微差异"而非"粗粒度区分"。从实验数据看，Emo SIM 提升幅度有限（+0.14 vs. cosyvoice），这一因果链存在跳跃。 |
| **数据支撑** | ✅ 通过 | 关键论断（DPO 提升情感可控性）有客观指标（Table I）和主观偏好测试（Fig. 3）双重支撑。消融实验（Table III）进一步支撑。 |
| **批判诚实** | ✅ 通过 | 已指出实验规模小、泛化性存疑、方法创新性有限、部分架构细节缺失等局限。 |
| **不填充** | ✅ 通过 | 未使用学术套话，每句均有实质信息。 |
| **区分事实与判断** | ✅ 通过 | 实验数据（如 98.87、85.6%）为事实；"DPO 能教会模型理解细微差异"为判断，已明确区分。 |
| **诚实** | ✅ 通过 | 对方法创新性、实验规模、可复现性的评价客观，未夸大贡献。 |

**额外检查与修改清单**：

1. **术语一致性**：全文统一使用"直接偏好优化（DPO）"、"指令微调（instruction tuning）"、"情感感知 LLM-TTS（Emotion-aware LLM-TTS）"。
2. **逻辑跳跃标注**：在"DPO 能捕捉细微差异"的论断旁标注了"假设"和"实验提升幅度有限"的批注。
3. **数据引用**：所有关键数字均标注来源表格/图表。
4. **推测标注**：关于"其他对比学习方法也可能达到类似效果"的论断已明确为推测。

---

## 9. 附注

- [?] 论文中提到的 JS 散度替代 log 比值的具体实现细节未给出，是否会影响 DPO 的理论保证？
- [?] Emotion-aware LLM-TTS 中情感信息具体注入到哪一层（输入嵌入层、注意力层、还是交叉注意力层）？论文描述不够详细。
- [?] 实验仅在 ESD 英文数据上进行，该方法在多语言、多 speaker、更多情感类别上的泛化性如何？
